...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#myTable').DataTable({
			...
			"ajax": {
				"url": "<?php echo site_url('UserController/get_data_user') ?>",
				"type": "POST",
			},
	...
	function updateList(id) {
		...
		$.ajax({
			url: "<?= base_url('UserController/get_id/'); ?>" + id,
			type: 'GET',
			dataType: 'JSON',
			success: function(data) {
				...
				if (data.avatar) {
					$('#label-avatar').text('Change Avatar'); // label avatar upload
					$('#avatar-preview div').html('<img src="' + base_url + 'upload/' + data.avatar + '" class="img-responsive mb-3 img-preview" style="width: 100px">'); // show photo					
				} else {
	...
	function save() {
		...
		if (save_method == 'add') {
			url = "<?php echo site_url('UserController/add'); ?>";
		} else {
			url = "<?php echo site_url('UserController/update/'); ?>" + url_id;
		}
