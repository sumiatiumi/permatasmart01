			...
			<div class="col-12">
				<div class="card">
					<div class="card-header">
						<h4>Table Siswa</h4>
						<div class="card-header-action"><a class="btn btn-primary" href="<?= base_url('admin/student/add'); ?>">Tambah Data</a></div>
					</div>
					<div class="card-body">
						<!--v-if-->
	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="POST" id="form" action="<?= base_url('AuthController/save_student'); ?>" enctype="multipart/form-data">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#myTable').DataTable({
			...
			"ajax": {
				"url": "<?php echo site_url('StudentController/get_data_user') ?>",
				"type": "POST",
			},
	...
	function updateList(id) {
		...
		$.ajax({
			url: "<?= base_url('StudentController/get_id/'); ?>" + id,
			type: 'GET',
			dataType: 'JSON',
			success: function(data) {
				...
				if (data.avatar) {
					$('#label-avatar').text('Change Avatar'); // label avatar upload
					$('#avatar-preview div').html('<img src="' + base_url + 'upload/img/student/' + data.avatar + '" class="img-responsive mb-3 img-preview" style="width: 100px">'); // show photo					
				} else {
	...
	function save() {
		...
		if (save_method == 'add') {
			url = "<?php echo site_url('StudentController/add'); ?>";
		} else {
			url = "<?php echo site_url('StudentController/update/'); ?>" + url_id;
		}
	...
	function deleteList(id) {
		swal({
			...
			if (result) {
				$.ajax({
					url: "<?= base_url('StudentController/delete/'); ?>" + id,
					type: "POST",
					dataType: "JSON",
					success: function(data) {
						reload_table();
